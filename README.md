...
[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce241952-wdt-interrupt-reset)

[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce241952-wdt-interrupt-reset](https://github.com/Infineon/mtb-example-ce241952-wdt-interrupt-reset)" application with the desired name "WdtInterruptReset" configured for the *KIT_PSOC4-HVMS-128K_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSOC4-HVMS-128K_LITE --app-id mtb-example-ce241952-wdt-interrupt-reset --user-app-name WdtInterruptReset --target-dir "C:/mtb_projects"
   ```
...
3. After programming, the application starts automatically. This code example deals with the watchdog timer (WDT) of PSoC&trade; 4. The WDT operates in two modes: the interrupt mode and the reset mode. In the interrupt mode, the LED toggles every second. If it's configured as the reset mode, the LED blinks thrice with an interval of 200 milliseconds each time the watchdog reset occurs.

   Signal name | KIT_PSOC4-HVMS-128K_LITE, KIT_PSOC4-HVMS-64K_LITE | KIT_PSOC4-HVPA-144K_LITE |  KIT_PSOC-HVPA-SPM1_LITE
   ---------|-----------|-----------|-----------------
	LED | LED7 (P0.6) | LED6 (P0.3) | LED6 (P0.5)

4. You can change the interrupt mode/the reset mode by changing the macro. 

**Figure 1. The macro to change the mode**
<BR><img src="./images/modes.png" width="410" /><BR>
<br>
...
3. LED toggles every second if it's the interrupt mode
4. LED blinks thrice each time the reset occurs if it's the reset mode
...
**Table 1. Application resources**
 ...
 :------- | :------------    | :------------ 
 WDT | WDT | To perform WDT
 GPIO | CYBSP_LED6 | Shows interrupt mode or reset mode
...
Document title: *CE241952* – *PSOC&trade; 4: Watchdog timer interrupt and reset*
